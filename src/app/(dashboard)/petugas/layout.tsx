...
const menuItems = [
  ...
  { id: "isi-data",        label: "ISI DATA KENDARAAN", icon: ClipboardList,   href: "/petugas/isi-data" },
  { id: "data-kendaraan",  label: "DATA KENDARAAN",     icon: Table2,          href: "/petugas/data-kendaraan" },
  { id: "analisis",        label: "ANALISIS",           icon: BarChart3,       href: "/petugas/analisis" },
  { id: "prediksi",        label: "PREDIKSI",           icon: Brain,           href: "/petugas/prediksi" },
  { id: "laporan",         label: "LAPORAN",            icon: FileText,        href: "/petugas/laporan" },
];
    ...
    <div className="flex min-h-screen bg-bg">
      ...
      <aside className={`${collapsed ? "w-20" : "w-64"} bg-sidebar min-h-screen flex flex-col fixed left-0 top-0 z-50 transition-all duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
        ...
        <button
          ...
          className="hidden md:flex absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-sidebar border-2 border-bg rounded-full items-center justify-center text-white/80 hover:text-white hover:bg-sidebar-hover z-50 shadow-lg"
        >
        ...
        <nav className={`flex-1 ${collapsed ? "px-2" : "px-4"} space-y-2 mt-4`}>
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center ${collapsed ? "justify-center px-2" : "gap-4 px-4"} py-3 rounded-lg transition-all duration-200 text-sm font-semibold tracking-wide text-left ${isActive ? "bg-white/10 text-accent" : "text-white/80 hover:bg-white/5 hover:text-white"}`}
              >
        ...
        <div className={`${collapsed ? "px-2" : "px-4"} pb-8`}>
        ...
        <header className="flex items-center justify-between px-4 md:px-8 py-4 bg-bg">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <div className="text-right">
              <p className="font-bold text-text-primary text-sm tracking-wide uppercase">{username || "PETUGAS"}</p>
